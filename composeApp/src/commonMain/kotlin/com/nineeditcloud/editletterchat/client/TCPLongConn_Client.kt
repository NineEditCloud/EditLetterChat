        ...
        while(coroutineContext.isActive){
            try{
                aSocket(SelectorManager(Dispatchers.IO) ).tcp().connect(host,port).use{ socket ->
                    ...
                    output=socket.openWriteChannel(autoFlush=true)/*输出流通道(发送)，autoFlush=true 启用自动刷新(flush)，适合场景是 连续多次写入、控制缓冲区、流式写入大文件，获取反馈值场景一次性用完后建议close()释放*/
...
//                            Log.msg("Ktor-TCP长连接", "收到服务器反馈：$line")
                    ...
                    }else if(authResponse.contains("authError") )/*若包含Token令牌错误*/
                        throw IllegalArgumentException("账号令牌认证失败: $authResponse,${authResponse.toHashMap()["msg"]}")/*抛出异常信息*/
                    else throw Exception("连接异常？: $authResponse")
                }
